...
function prcdata = rm_instrum_resp(rawdata,badvals,samplrate,pols,zers,flo,fhi,ordl,ordh,digout,digoutf,ovrsampl,idelay)
    ...
    if (sum(isnan(ytl)) > 0 || (mxll/len) > 0.9 )
        error('Low pass Butterworth filter is unstable, decrease ord h')
    else
    end
    % check if high pass butterworth filter is stable
    if (sum(isnan(yth)) > 0 || (mxlh/len) > 0.9 )
        error('High pass Butterworth filter is unstable, decrease ord l')
    else
    end
