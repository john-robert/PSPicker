...
function event2nor_single(EVENT,sfile_name)
    ...
    nor_struct.typemag        = 'L';
    nor_struct.magagency      = EVENT.AGENCY;
    nor_struct.ERROR.x        = EVENT.XERR;
    nor_struct.ERROR.y        = EVENT.YERR;
    nor_struct.ERROR.z        = EVENT.ZERR;
    nor_struct.rms            = EVENT.RMS;
    nor_struct.WAV            = EVENT.WAV;
    nor_struct.agency         = EVENT.AGENCY;
    nor_struct.ID             = EVENT.ID;
    ...
    for i=1:num_arrivals
        if ~isempty(EVENT.PHASES(i).ARRIVAL)
            ...
            DATA(k).instrument= EVENT.PHASES(i).INSTRUMENT;
            DATA(k).component = EVENT.PHASES(i).COMPONENT;
