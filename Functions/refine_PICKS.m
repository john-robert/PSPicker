...
function S=refine_PICKS(EVENT,mainfile,flag_plot_level)
    ...
    EVENT          = comp_THEO(PickerParam,hyp,EVENT,flag_plot_level);
    ...
    for iter=1:length(DATA) % iter goes trough all the stations (for one iter we can analyse all the 4 components)
        ...
        %%% Select traces that passed the tests
        flag_levels = boolean_presnr & boolean_gaussian & boolean_kurto;

        if ~any(flag_levels) % if all are 0
            fprintf(1,'   refine P-Picks: Station %s skipped. Pre-tests not passed.\n', station);
            continue
        else
        ...
        for j=1:numel(DATA_P.RAW)
            ...
            if isempty(ind_pick)
                fprintf(1,'   WARNING: no P-phase found for station %s\n',station);
                continue
            end
            ...
            [snr,max_ind,max_snr] = snr_function( abs(fil_trace_P), rsample, LTA_snr, STA_snr, smooth_snr ,ind_P_n(j)-window_snr(1)*rsample,ind_P_n(j)+window_snr(2)*rsample, flag_plot);
            max_snr_n(j)          = max_snr;
        ...
        if strcmp(PickerParam.weight_switch,'SNR')
            boolean_snr_P   = (max_snr_n>=PickerParam.SNR2W_P(1) & max_snr_n==max(max_snr_n));
            ...
            if ~any(boolean_snr_P)
                ...
                [val, idx]  = max(max_snr_n);
                instr_code_P= DATA_P.RAW(idx).CHAN(1);
                comp_code_P = DATA_P.RAW(idx).COMP;
            end

        else
            boolean_kurto_P = (max_kurto_n>=PickerParam.KURTO2W_P(1) & max_kurto_n==max(max_kurto_n));
            ...
            if ~any(boolean_kurto_P)
                ...
                [val, idx]  = max(max_kurto_n);
                instr_code_P= DATA_P.RAW(idx).CHAN(1);
                comp_code_P = DATA_P.RAW(idx).COMP;
            end
        end

        fprintf(1, '   refine P-picks: Station %s, code %s%s, SNRmax %5.1f, weight %1d\n',station, instr_code_P, comp_code_P, max(max_snr_n), weight_P);
        ...
        if pick_flag_S==1;
            ...
            if ~any(boolean_select)
                pick_flag_S=0;
                fprintf(1,'   refine S-Picks: Station %s skipped. Pre-tests not passed.\n', station);
                continue
            else
           ...
           [all_mean_M,M,CF_time]=trace2FWkurto(cf_S,rsample,...
            ...
            [snr,max_ind,max_snr] = snr_function( abs(cf_S), rsample, LTA_snr, STA_snr, smooth_snr, ind_S-window_snr(1)*rsample, ind_S+window_snr(2)*rsample, flag_plot(3) );
            max_snr_n             = max_snr;
            ...
            if strcmp(PickerParam.weight_switch,'SNR')
                ...
                boolean_snr_S = ( max_snr_n>=PickerParam.SNR2W_S(1) & max_snr_n==max(max_snr_n) );
                if ~any(boolean_snr_S)
                    ...
                    ind_S        = ind_S;
                    [val, idx]   = max(max_snr_n);
                    instr_code_S = DATA_S.RAW(idx).CHAN(1);
                    comp_code_S  = DATA_S.RAW(idx).COMP;
                end

            else

                boolean_kurto_ S = ( max_kurto_n>=PickerParam.KURTO2W_S(1) & max_kurto_n==max(max_kurto_n) );
            ...
            fprintf(1, '   refine S-picks: Station %s, code %s%s, SNRmax %5.1f, weight %1d\n',station, instr_code_S, comp_code_S, max(max_snr_n), weight_S);
        end
        ...
        % for P
        PHASES(ind_Pphase).INSTRUMENT     = instr_code_P;
        PHASES(ind_Pphase).COMPONENT      = comp_code_P;
        ...
        % for S
        if pick_flag_S==1
            PHASES(ind_Sphase).INSTRUMENT = instr_code_S;
            PHASES(ind_Sphase).COMPONENT  = comp_code_S;
    ...
    %%% Relocate 
    new_event       = comp_THEO(PickerParam, hyp, EVENT, flag_plot_level);
