			...
			for comp_s in "${comp[@]}"; do 
				...
				for network_s in "${network[@]}"; do 
					...
					for data_path in "${data_path[@]}"; do
						...
						#echo "$station"
						#echo $data_path$wav_name

						for file in $true_file_list ; do
...
echo
echo 'sds2mseed.sh'
echo "dataselect @$list_file -ts "$dataselect_start" -te "$dataselect_end" -o $output"
dataselect -v @$list_file -ts "$dataselect_start" -te "$dataselect_end" -o $output
