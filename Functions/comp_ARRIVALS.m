...
function comp_ARRIVALS(params,hyp,sfile,sfile_output,show)
    ...
    %%% Check which STATION.HYP should be used and check if exists in current directory
    ...
    [filepath,name,ext] = fileparts(params.station_file);          % to get absolute path, so .HYP file is not exptected in current directory (usually directory of 'main.m')
    model_file          = [filepath,'/STATION',mod_name,ext];
